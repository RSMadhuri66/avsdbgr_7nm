...
1. **PTAT Current Calculation**  
   The Proportional to Absolute Temperature (PTAT) current \(I_{PTAT}\) is generated by the voltage difference across two resistors \(R_1\) and \(R_2\). The PTAT current is given by:

   \[
   I_{PTAT} = \frac{\Delta V_T}{R_1} = \frac{V_T \cdot \ln(N)}{R_1}
   \]
...
2. **CTAT Voltage Calculation**  
   The Complementary to Absolute Temperature (CTAT) voltage \(V_{CTAT}\) decreases with temperature:

   \[
   V_{CTAT} = V_{BE} = V_{BE0} - \beta T
   \]

   Where:
   - \(V_{BE0}\) = Base-emitter voltage at absolute zero temperature
   - \(\beta\) = Temperature coefficient of \(V_{BE}\) (typically \(-2 \, \text{mV/K}\))
...
4. **Resistor Values**  
   The resistor values \(R_1\) and \(R_2\) are critical in determining the ratio of PTAT to CTAT components:
...
6. **Current Mirror Relationships**  
   In current mirrors, the output current \(I_{out}\) is proportional to the transistor sizing:
