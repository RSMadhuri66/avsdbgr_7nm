...
# Bandgap Reference Circuit Equations

1. **PTAT Current Calculation**  
   The Proportional to Absolute Temperature (PTAT) current I_PTAT is generated by the voltage difference across two resistors R1 and R2. The PTAT current is given by:

   I_PTAT = ΔV_T / R1 = (V_T * ln(N)) / R1
   ...
   - R1 = Resistor value used in the PTAT generation branch

2. **CTAT Voltage Calculation**  
   The Complementary to Absolute Temperature (CTAT) voltage V_CTAT decreases with temperature:

   V_CTAT = V_BE = V_BE0 - βT

   Where:
   - V_BE0 = Base-emitter voltage at absolute zero temperature
   - β = Temperature coefficient of V_BE (typically -2 mV/K)
   - T = Temperature in Kelvin

3. **Reference Voltage Calculation (V_ref)**  
   The reference voltage is obtained by combining the PTAT and CTAT voltages:

   V_ref = V_CTAT + k * V_PTAT

   Where:
   - k = R2 / R1 (Scaling factor determined by the resistor ratio)

4. **Resistor Values**  
   The resistor values R1 and R2 are critical in determining the ratio of PTAT to CTAT components:

   R2 = k * R1

5. **Startup Circuit Currents**  
   The current through the startup transistors can be calculated by:

   I_startup ≈ (V_gs - V_th) / R_startup

   Where:
   - V_gs = Gate-source voltage of the startup transistor
   - V_th = Threshold voltage of the nMOS transistor
   - R_startup = Resistance in the startup circuit path

6. **Current Mirror Relationships**  
   In current mirrors, the output current I_out is proportional to the transistor sizing:

   I_out = (W/L)_out * (W/L)_ref * I_ref

   Where:
   - I_ref = Reference current set by the current mirror
